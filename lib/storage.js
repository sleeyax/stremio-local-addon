...
function Storage(opts) {
	...
	this.opts.entryIndexes.forEach((key) => self.indexes[key] = new Map())
			...
			return new Promise((resolve) => {
				...
				.on('finish', () => {
					...
					.then((fd) => {
						...
						if(truncate) {
							self.indexes.primaryKey.forEach((entry, key) => persistEntry(key, entry))
						}
					})
					.catch(onInternalErr)
					.then(resolve)
	...
	function commitEntry(key, entry) {
		...
		self.opts.entryIndexes.forEach((property) => {
