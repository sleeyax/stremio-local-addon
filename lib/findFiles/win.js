...
function findFilesWin() {
	...
	propsProc.on('error', (err) => ev.emit('err', err))
	propsProc.stdout.pipe(byline()).on('data', (line) => ev.emit('file', line.toString().trim()))
	propsProc.stderr.on('data', (chunk) => console.log('powershell search: '+chunk.toString()))
	propsProc.on('close', () => ev.emit('finished'))
